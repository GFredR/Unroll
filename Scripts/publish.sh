...
#           泄漏扫描**三个通道**:① 受控文件的当前内容、② 提交消息、
#           ③ 全部历史对象的内容与路径(含已删文件 / dangling / 二进制)
# ============================================================================
set -euo pipefail
...
# 第三个通道:**历史对象的内容与路径**(2026-09-24 补上)。
# 前两道都只看"现在"——① 受控文件的当前内容、② 提交消息 —— 而它们共同的盲区是:
# **已删文件的历史内容**(当前工作区干净得看不出问题)与**二进制里的字符串**
...
if [ "$HIST_RC" -eq 0 ]; then
    ...
    HIST_COUNT="$(printf '%s\n' "$HIST_OUT" | sed -n 's/.*扫了 \([0-9]*\) 个 blob.*/\1/p' | head -1)"
    ok "历史对象未命中(含已删文件 / dangling / 二进制,扫了 ${HIST_COUNT:-?} 个 blob)"
elif [ "$HIST_RC" -eq 1 ]; then
    fail "历史对象里命中泄漏关键词(内容刻意不打印):"
    printf '%s\n' "$HIST_OUT" >&2
else
    fail "历史对象扫描未能完成 —— 前置不满足(缺 python3 / 不在 git 仓库)"
    printf '%s\n' "$HIST_OUT" >&2
fi
