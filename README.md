...
- **Archives that hold more than images** — a `ComicInfo.xml`, a `readme`, macOS `__MACOSX` packing junk and `thumbs/` thumbnails can all sit next to the pages. Non-image entries are dropped silently and page numbering starts at the first real image. Entries named in a way that is plainly another format (`.txt`, `.xml`, `.pdf`) are left out too — but **a page with no extension at all is kept** (old scan packs name them `001`, `002`): the page's own bytes decide, not its name, so such an archive opens rather than being reported as containing no images. A page that still turns out not to be an image degrades to a single failed-page card; the rest of the archive is unaffected.
- **Zero third-party dependencies** — it uses the `libarchive` (BSD-2) that already ships with macOS. The whole app is about 2.4 MB: a 1.75 MB main binary plus 564 KB for the two Finder extensions.
...
- **Opens on the thumbnail grid** — picking an archive lands you on the whole book at once; click any cell to start reading there, and `Esc` (or the grid button in the HUD, `⇧⌘G`) takes you back to it. Every previous shortcut assumed you already knew where you were going; the grid is the answer to "which page was that spread on again?". It generates in **one sequential pass** (not "generate whatever you scroll to" — that degrades to quadratic on solid 7z), shows progress, and can be stopped at any time. If it hits the memory ceiling it **says so**, rather than leaving you waiting for pages that will never appear. Opening another archive or regenerating discards the previous batch. Switching between the grid and the reader never restarts or cancels a scan in flight.
...
- **A way back you can always see** — the reading view keeps a single-column thumbnail rail down the left (the whole volume, current page outlined) with a permanent **‹ All pages** button at its top, and the canvas carries page arrows on both edges. When the mouse goes quiet these **dim to 45% instead of disappearing** — you can always see where they are, and moving the mouse brings them back to full strength. The arrows follow your reading direction: under right-to-left order, "next page" is the left one. (Continuous scroll has no spreads, so the arrows step aside there — the rail stays.)
...
- **Export every page of the volume** (`⇧⌘E`) — writes **every page in the archive** to image files in reading order, **passing the original bytes through untouched**: no decoding, no re-encoding, no compositing. It is one half of a pair with `⌘S`: `⌘S` saves **the spread you are looking at** (two pages become one image, JPEG gets re-encoded) while `⇧⌘E` saves **every page in the archive** — ready to be used as wallpapers, a translation reference board, or something to edit further. Files are named `<archive>-p003.png`, so they sort in reading order; the extension comes from the entry's own name when it has one, otherwise from the file's magic number, falling back to `.bin` rather than guessing `.jpg`. Encrypted pages are **skipped, not counted as damage** (the panel reports "N encrypted pages skipped" and "N pages unreadable" separately), and a failed write (no permission / no space) **stops immediately and says the write failed** instead of ploughing through the rest and reporting "200 pages failed". You can stop at any time — files already written are kept.
...
- **Window size and position are remembered** — quit, relaunch, and the window comes back where you left it. The saved frame is clamped to a screen that still exists, so unplugging an external display brings the window back into view instead of leaving it off-screen. Only four numbers are stored — no path, no file name. Minimum window size is 900×600.
...
Sitting on page 1 of a **437 MB** archive, the shipped app holds **183 MB** resident. The app is **2.4 MB** on disk (a 1.75 MB main binary plus 564 KB for the two Quick Look extensions); the DMG is **1036 KB** (1.1.0's was 972 KB).
...
| `⇧⌘G` | Back to the thumbnail grid (an archive opens on it; `Esc` also returns) |
| `⌘S` | Save the current page as an image (whole spread in two-page mode) |
| `⇧⌘E` | Export every page of the volume — reading order, original bytes untouched (no re-encoding, no compositing) |
