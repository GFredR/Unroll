...
| `UnrollTests` (app-hosted) | Yes | 216 | View model logic, layout/direction (including the continuous-scroll viewport/page-number link and its active-row window), progress and bookmark persistence, recents, localisation keys, menu/shortcut guards, the thumbnail grid's scan-and-budget rules, the bulk-export glue (naming, on-disk order, write-failure mapping), and the memory-budget invariants themselves |

311 cases / 309 passing / 2 skipped as of 2026-09-22 (the skips are the external-sample and synthetic-benchmark cases, both opt-in). Counts are de-duplicated by test name — the raw log prints two lines per case.
