...
> **发布状态（2026-09-22）**：**本机已冻结** —— 产物已按 `00e8062` 重建（App 2.18 MB / DMG 972 KB，Universal 2，ad-hoc 签名），`v1.1.0` tag 已打，`publish.sh` 前检只剩「未登录 `gh`」一项。**尚未推送**（仓库当前没有远程）。冻结发布内容前请先确认下面四条都在。
