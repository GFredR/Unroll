...
> **发布状态（2026-09-22 冻结，2026-09-23 复核）**：**本机已冻结** —— 产物已按 `00e8062` 重建（App 2.18 MB / DMG 972 KB，Universal 2，ad-hoc 签名），`v1.1.0` tag 已打。**尚未推送**（仓库当前没有远程）。⚠️ **冻结那一刻**前检只剩「未登录 `gh`」一项；但冻结之后又落了源码提交（导出面板取证、可发现性三件套），所以现在跑前检会按设计多报两条红 —— `[4/8] v1.1.0 之后还有源码改动`（12 个产物路径内的文件）与 `[5/8] 产物不是当前源码构建的（产物 00e8062 / 源码 042db86）`。**这是设计，不是故障**：判据比的是提交号、不是内容，产物本身仍与它的哈希自洽（`dirty=0`、侧车哈希 == DMG 哈希）。下次发版升版本号走完整流程即可，**别 `git tag -f`**。冻结发布内容前请先确认下面四条都在。
