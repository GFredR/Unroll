...
- **零第三方依赖**——用的是 macOS 自带的 `libarchive`（BSD-2 许可）。整个 App 约 2.18 MB：主程序 1.61 MB，两个 Finder 扩展加起来约 0.5 MB。
...
- **缩略图网格**（`⇧⌘G`）——一眼看全卷，点任一格直达该页。此前所有快速移动手段都假设"你已经知道要去哪"，想找某一页只能一张张翻着认。生成过程**一趟顺序扫完**（不是"滚到哪生成哪"，那在 solid 7z 上会退化成平方级），带进度条、可随时停止；内存到顶时会**明说**"后面的页不会有图"，而不是让你一直等。换书或重新生成都会作废旧的一批。
...
打开 **437 MB** 的归档停在第一页，发布版 App 常驻 **183 MB**；App 在磁盘上 **2.18 MB**（主程序 1.61 MB + 两个 QuickLook 扩展约 0.5 MB），DMG **972 KB**（上一版为 812 KB）。
