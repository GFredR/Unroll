...
- **包里混着别的东西也照常打开**——`ComicInfo.xml`、`readme`、macOS 的 `__MACOSX` 打包垃圾、`thumbs/` 缩略图都可以和页图混在同一个包里：非图片条目静默丢弃，页序从第 1 张真图算起。名字一看就是别的格式的条目（`.txt` / `.xml` / `.pdf`）也一并排除；但**完全没有扩展名的条目会留下**（老式扫描包常把页名写成 `001`、`002`）——由这一页自己的字节决定，不由名字决定，所以这种包能正常打开，而不是报「归档内没有找到图片」。万一某一页确实不是图片，它只降级成**一张失败卡片**，不影响其余页。
- **零第三方依赖**——用的是 macOS 自带的 `libarchive`（BSD-2 许可）。整个 App 约 2.4 MB：主程序 1.75 MB，两个 Finder 扩展加起来 564 KB。
...
- **页码直达**——`⌥⌘G` 输入页码跳转；输入时就地显示那一页，输错当场看得见。
- **打开就落在缩略图网格上**——选中一个包先看到整卷；点任一格从那里开始读，`Esc`（或 HUD 上的网格按钮 `⇧⌘G`）再回到网格。此前所有快速移动手段都假设"你已经知道要去哪"，想找某一页只能一张张翻着认。生成过程**一趟顺序扫完**（不是"滚到哪生成哪"，那在 solid 7z 上会退化成平方级），带进度条、可随时停止；内存到顶时会**明说**"后面的页不会有图"，而不是让你一直等。换书或重新生成都会作废旧的一批；在网格与阅读之间来回切，不会重启、也不会取消正在跑的那一轮扫描。
- **连续滚动**（`⌘0`）——竖向一列，一页接一页，适合**顺着读**而不是「一页页翻」。滚动位置与页码是**同一份状态**：视口驱动页码、跳页也会带动视口，两者不可能对不上。只有视口附近的几行保留全分辨率图，出窗的行会主动把像素交还 —— 这才是长距离滚动仍守在内存预算内的原因。缩放四档与「封面单独一页」描述的是**摊怎么放进窗口**，在滚动模式下没有意义，会一并灰掉。
- **一条永远看得见的回头路**——阅读层左侧常驻一列缩略图（整卷，当前页带描边），栏顶是常驻的「‹ 全部页面」按钮；画布两侧还有翻页箭头。鼠标静置时这些控件**淡化到 45% 而不是消失**——位置一直看得见，鼠标一动立刻恢复全亮。箭头语义跟随阅读方向：右开时「下一页」在左边。（连续滚动没有「摊」这个概念，那一档下箭头让位、左栏保留。）
- **当前页另存为图片**（`⌘S`）——把正在看的这一页存成 PNG 或 JPEG；双页模式下导出的是**屏幕上那一整摊**（按当前阅读方向并排），不是孤零零一页。
- **导出本卷全部页**（`⇧⌘E`）——把**这一卷的每一页**按阅读顺序导成图片文件，**原始字节直通**：不解码、不重新编码、不合成。与 `⌘S` 是一对分工：`⌘S` 存「你看到的那一摊」（双页并成一张、JPEG 会重编码），`⇧⌘E` 存「归档里的每一页」（拿去做壁纸 / 翻译参考板 / 二次加工）。文件名是「归档名 + 补零页号 + 扩展名」（`vol01-p003.png`），排出来就是阅读顺序；扩展名优先取条目原名，原名给不出时按文件头判断，认不出来就如实标 `.bin` 而不是猜一个 `.jpg`。加密页**跳过而不算损坏**（面板把"跳过几页加密页"与"几页读不出来"分开说）；**写不出去会立刻停下并如实说是写入失败**，不会接着把剩下的页读完再报一句"200 页全失败"。可以随时停止，已写出的文件保留。
...
- **欢迎页，而不是一个空窗口**——空态会列出**你最近打开的书，连同上次读到第几页**（`vol01.cbz · P.12/48`），外加一行你真正会用到的快捷键。这些记录 App 一直在存，只是从来没在空窗口上显示过。
- **帮助菜单**（`⌘?`）——全部快捷键一张清单，其中**菜单里根本没有对应项的五个画布手势**单独成组：那是 App 里唯一能告诉你它们存在的地方。第一次打开一本归档时，页面上方会出现一行可关闭的提示——**只自动出现一次**，之后随时可以从「帮助 → 显示阅读提示」叫回来。这一行是**布局里的一行、不是浮层**，所以页面拿到的是扣掉它之后的高度，从结构上就不可能被盖住。
- **窗口尺寸与位置会被记住**——退出再打开，窗口回到你离开时的位置。存下来的框会被夹到**仍然存在的那块屏**上：拔掉外接屏后窗口会回到可见范围，而不是留在屏外。只存**四个数字**——不含路径、不含文件名。最小窗口 **900×600**。
...
打开 **437 MB** 的归档停在第一页，发布版 App 常驻 **183 MB**；App 在磁盘上 **2.4 MB**（主程序 1.75 MB + 两个 QuickLook 扩展 564 KB），DMG **1036 KB**（上一版为 972 KB）。
...
| `⇧⌘↑` | 回到封面 |
| `⇧⌘↓` | 跳到末页 |
| `⌥⌘G` | 跳转到指定页（输入页码时就地预览那一页） |
| `⇧⌘G` | 回到缩略图网格（打开归档即落在网格上；`Esc` 也能返回） |
| `⌘S` | 当前页另存为图片（双页时导出整摊） |
| `⇧⌘E` | 导出本卷全部页——按阅读顺序、原始字节直通（不重编码、不合成） |
| `⇧⌘F` | 在访达中显示当前文件 |
...
这些边界划分背后的理由——以及促成它们的那几个决策——见 [ARCHITECTURE.zh-CN.md](ARCHITECTURE.zh-CN.md)。
