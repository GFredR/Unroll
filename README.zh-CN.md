...
- **零第三方依赖**——用的是 macOS 自带的 `libarchive`（BSD-2 许可）。整个 App 约 1.9 MB：主程序 1.3 MB，两个 Finder 扩展加起来约 0.5 MB。
...
打开 **437 MB** 的归档停在第一页，发布版 App 常驻 **183 MB**；App 在磁盘上 **1.9 MB**（主程序 1.3 MB + 两个 QuickLook 扩展约 0.5 MB），DMG **812 KB**。
