...
enum RecentPresentation {
    ...
    ///
    /// 原先取 4 的**依据**是窗口最小高度(`minWidth: 720, minHeight: 480`):
    /// 空态改成「图标 96 + 标题块 82 + 按钮 32 + 小抄 16 + 最近 N 条」之后,
    /// 按 4 条算是 465pt,5 条是 481pt —— 后者**刚好越界 1pt**,窗口收到最小时
    /// 底部会被裁掉半行。留 15pt 余量,不做"刚好放得下"的设计。
    ///
    /// ⚠️ **2026-09-23:这条依据已过期** —— 同批为「左侧缩略图栏」把最小窗口
    /// 抬到 `DesignSystem.Window` 的 880×600(2026-09-24 又抬到 900×600,只动宽度),
    /// 空态因此多出 120pt,5 条也放得下了。
    /// **上限仍保持 4**,但理由退回上面那句更稳的话(欢迎页不是管理器)——
    /// 而不是"放不下"。数字没动,动的是它背后的道理;把这条写在这里是因为
    /// **一条失去依据的数字比一个错数字更危险**:它看起来仍然有出处
