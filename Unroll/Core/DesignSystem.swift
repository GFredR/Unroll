...
enum DesignSystem {
    ...
    enum PageBudget {
        ...
        /// 连续滚动模式下「锚点两侧各保留几行全分辨率」(2026-09-21)。
        ///
        /// **为什么需要它**:`LazyVStack` 只是「不渲染」看不见的行,行内 `@State`
        /// 攥着的 `CGImage` 引用计数**在我们手上,不在 `PageCache` 的预算账本里**
        /// —— 于是「SwiftUI 会回收」这句话并不能当内存上限用。窗口把「行内持有」
        /// 钉成一个可数的数:同时至多 `2 × 3 + 1 = 7` 行,小于全分辨率池本来的
        /// `maxCachedPages = 8`。
        ///
        /// 这条关系是**不变量**,由 `AppSkeletonTests` 断言 —— 调大窗口
        /// (`5` 就变成 11 行)会当场被拦下,而不是等到有人在 300 页的包上
        /// 看着活动监视器才发现。
    ...
    enum Window {
        ...
        /// **最小尺寸**:抬过两次,两次的理由都记在这儿(别让后一次覆盖掉前一次)——
        /// ① 2026-09-23 由 720×480 抬到 880×600:不是那个 4/3,而是同批新增的
        ///    左侧缩略图栏 —— 栏占 `Chrome.railWidth`(112pt),最小窗口若仍是 720,
        ///    画布只剩 608,**比加栏之前的整套 720 还窄**。880 让画布回到 768。
        /// ② 2026-09-24 由 880×600 抬到 **900×600**(用户指定):画布回到 788。
        ///    用户给的是一对数字,只有宽度变了 —— 高度 600 两次不变,说明它不是
        ///    "随手乘出来的",而是"放下 5 条最近打开"那条版式判断(见 `RecentPresentation`)
        /// 这条不变量由 `AppSkeletonTests` 锁住 —— 判据是**关系**(扣掉栏之后 ≥ 720),
        /// 不是这两个具体数字:数值随版式调,关系一破就退回用户抱怨的那个状态
