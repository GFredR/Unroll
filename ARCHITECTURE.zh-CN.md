...
**关于验证的诚实说明。** 算法层和视图模型层有 20 个单元测试覆盖，其中包括两条失败分类规则（加密页算**跳过**、不算损坏；解码失败**不**重开扫描器，而字节读取失败**必须**重开）。原先这里写的是「**面板在真实窗口里长什么样，没有任何自动化证据**」—— **这句已于 2026-09-20 过期并被收回**：改由**仅 DEBUG 编译的 App 内驱动**（`DemoDriver`，标记文件名即场景名）自己打开面板，`verify-ui.sh` 的 `ONLY=grid` / `ONLY=jump` 两段写下 5 条结构化判据并留下截图。真实窗口外部的按键注入**仍然**被 TCC 挡住 —— 这正是必须让 App 自己驱动的原因，而不是"没有证据"的原因。**仍没验的**只剩下手感类：生成速度、`budgetReached` 的界面文案（要 600+ 页样本才触发）。
...
| `UnrollTests`（宿主为 App） | 是 | 238 | 视图模型逻辑、版式／方向（含连续滚动的「视口 ↔ 页码」联动与活跃行窗口）、进度与书签持久化、最近打开、本地化 key、菜单与快捷键守卫、缩略图网格的扫描与预算规则、批量导出的粘合层（命名、落盘顺序、写失败映射）、**内存预算的不变量本身**，以及只出现一次的阅读提示标记（按「只存一个布尔」断言）、菜单／空态／帮助面板三处共用的最近打开展示层与快捷键清单 |

截至 2026-09-22：**333 用例 / 331 通过 / 2 skip**（两个 skip 是外部样本与合成基准用例，都需要手动开启）。计数按用例名去重——原始日志每个用例会打两行。**用 Release 配置跑全量单测在设计上不可能**：`ArchiveKitTests` 用了 `@testable import`，它需要 `-enable-testing`（只有 Debug 有），所以「零警告」标准里的 Release 那一半指的是**完整构建**，不是跑测试。
