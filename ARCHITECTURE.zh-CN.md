...
| `UnrollTests`（宿主为 App） | 是 | 200 | 视图模型逻辑、版式／方向（含连续滚动的「视口 ↔ 页码」联动与活跃行窗口）、进度与书签持久化、最近打开、本地化 key、菜单与快捷键守卫、缩略图网格的扫描与预算规则，以及**内存预算的不变量本身** |

截至 2026-09-21：**287 用例 / 285 通过 / 2 skip**（两个 skip 是外部样本与合成基准用例，都需要手动开启）。计数按用例名去重——原始日志每个用例会打两行。
