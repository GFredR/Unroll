...
| `UnrollTests`（宿主为 App） | 是 | 242 | 视图模型逻辑、版式／方向（含连续滚动的「视口 ↔ 页码」联动与活跃行窗口）、进度与书签持久化、最近打开、本地化 key、菜单与快捷键守卫、缩略图网格的扫描与预算规则**及其与阅读层的双向往返**（打开归档落在网格层、`Esc` 返回网格层，切层既不重启也不取消在途扫描）、批量导出的粘合层（命名、落盘顺序、写失败映射）、**内存预算的不变量本身**，以及只出现一次的阅读提示标记（按「只存一个布尔」断言）、菜单／空态／帮助面板三处共用的最近打开展示层与快捷键清单，以及阅读层周边控件与窗口几何的不变量（淡化永远不会把「回去的路」藏掉、箭头点击区守住 HIG 下限、缩略图栏永远不会把画布压到比旧的最小窗口更窄） |

截至 2026-09-23：**337 用例 / 335 通过 / 2 skip**（两个 skip 是外部样本与合成基准用例，都需要手动开启）。计数按用例名去重——原始日志每个用例会打两行。**用 Release 配置跑全量单测在设计上不可能**：`ArchiveKitTests` 用了 `@testable import`，它需要 `-enable-testing`（只有 Debug 有），所以「零警告」标准里的 Release 那一半指的是**完整构建**，不是跑测试。
