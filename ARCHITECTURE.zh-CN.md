...
开卷小到一个下午就能通读一遍，但真正定基调的是少数几个决策。本文件把它们记下来：问题是什么、考虑过哪些替代方案、最后为什么选了这个。这里的数字都是**实测**的，不是估计——每一个都来自已入库的 fixture 或一次脚本运行，没有一个是估的。
...
**关于验证的诚实说明。** 算法层和视图模型层有 20 个单元测试覆盖，其中包括两条失败分类规则（加密页算**跳过**、不算损坏；解码失败**不**重开扫描器，而字节读取失败**必须**重开）。原先这里写的是「**面板在真实窗口里长什么样，没有任何自动化证据**」—— **这句已于 2026-09-20 过期并被收回**：改由**仅 DEBUG 编译的 App 内驱动**（`DemoDriver`，标记文件名即场景名）自己打开面板，`verify-ui.sh` 的 `ONLY=grid` / `ONLY=jump` 两段写下 5 条结构化判据并留下截图（5 条是 2026-09-20 写下这句时的数；**2026-09-23 起两段合计 8 条**，其中 `ONLY=grid` 一段自己就有 5 条，多出来的那条断言的是窗口确实回到了网格层 —— `layer=browse`）。真实窗口外部的按键注入**仍然**被 TCC 挡住 —— 这正是必须让 App 自己驱动的原因，而不是"没有证据"的原因。**仍没验的**只剩下手感类：生成速度、`budgetReached` 的界面文案（要 600+ 页样本才触发）。

## 决策 8 —— 批量导出的遍历放在 Kit 里，命名与写盘不

**问题。** `⇧⌘E` 要按阅读顺序走遍每一页，并把它的**原始字节**交出去。最容易悄悄坏掉的两件事是：顺序（归档的物理顺序不是阅读顺序 —— `plain.cbz` 里 `page10` 排在 `page1` 前面）和字节（有人"顺手"解码再重编码，「把原素材取出来」这件事就坏了，而且**没有任何症状** —— 图还是能看）。

**为什么这个循环不放在 App 层。** 要断言上面这两件事，需要仓库里的 fixture，而宿主为 App 的那个测试 target 跑在**沙盒里**，可能压根读不到它们（`PageStoreTests` 与 `IntegrityCheckTests` 都为此写了 skip 分支）。在 `ArchiveKit` 里，`swift test` 是无沙盒的，能把交付出的字节与 fixture 逐字节对拍。

**于是切法是：** `PageSequenceExtractor` 归 Kit（遍历、加密／损坏分类、取消、受控重开 —— 与 `ArchiveIntegrityChecker` 同一副骨架，且它**自己不写任何文件**）；App 层只负责给文件起名并写盘。写盘失败由 `onPage` 返回 false 回传，Kit 把它记成 `.sinkStopped` —— 刻意与"损坏"分开，于是面板说的是"写入失败"，而不是让压缩包背锅。

**而输出路径从头到尾不碰条目名。** 一个文件是 `<归档名>-p003.png`：归档名、补零页号、扩展名，没有第四样东西 —— 不还原目录树、不还原时间戳、不还原权限位、不还原符号链接。这是**安全属性**而不是偷懒：zip-slip 不可能发生，因为条目名从未被当成路径用过。
...
| 把归档解压到磁盘**当作阅读方式** | 这是整个项目的前提。**解压正是这个 App 存在的意义所要避免的事**。注意与 v1.1 新加的 `⇧⌘E`「导出本卷页文件」的区别：那个**确实是**用户主动要求的写文件 —— 但它不是解压：写出的是一批按「归档名 + 补零页号」命名的平铺文件、按阅读顺序，其余一概不还原（不还原目录树、不还原时间戳、不还原权限位、不还原符号链接）。也正因为如此它**不可能发生 zip-slip**：条目名从头到尾没有被当成路径用过 |
...
| --- | --- | --- | --- |
| `ArchiveKit`（SwiftPM） | 否 | 78 | 格式处理、自然排序、顺序读取、完整性扫描、批量页遍历、逐字节 fixture 比对 |
| `UnrollLogicTests` | 否 | 6 | 阅读器对 `ArchiveKit` 的契约，毫秒级跑完 |
| `UnrollQuickLookTests` | 否 | 11 | Quick Look 扩展的读页逻辑：首页选取、降采样上限、以及完整的失败态映射 |
| `UnrollTests`（宿主为 App） | 是 | 260 | 视图模型逻辑、版式／方向（含连续滚动的「视口 ↔ 页码」联动与活跃行窗口）、进度与书签持久化、最近打开、本地化 key、菜单与快捷键守卫、缩略图网格的扫描与预算规则**及其与阅读层的双向往返**（打开归档落在网格层、`Esc` 返回网格层，切层既不重启也不取消在途扫描 —— **含「已有续读记录」那一种情形**：续读会恢复页码与版式，但仍落在网格层，绝不直接进阅读层）、批量导出的粘合层（命名、落盘顺序、写失败映射）、**内存预算的不变量本身**，以及只出现一次的阅读提示标记（按「只存一个布尔」断言）、菜单／空态／帮助面板三处共用的最近打开展示层与快捷键清单，以及阅读层周边控件与窗口几何的不变量（淡化永远不会把「回去的路」藏掉、箭头点击区守住 HIG 下限、缩略图栏永远不会把画布压到比旧的最小窗口更窄），以及**自管窗口记忆的「纯」那一半** —— 判据函数（存下来的框会被抬到最小尺寸、被按到仍然拥有它的那块屏内；屏幕没了就把窗口拉回；并列时取法确定）与存储的往返（记录缺失或损坏时给 `nil` 而不是抛异常）。这段记忆**有没有在正确的时机被读、被写**是刻意不放进单测的；它由 `Scripts/probe-window-memory.sh` 端到端取证 |

截至 2026-09-24：**355 用例 / 351 通过 / 4 skip**（其中 2 个是 opt-in——外部样本要 `UNROLL_EXTERNAL_SAMPLES`、合成基准要 `UNROLL_BENCH_FIXTURE`；另 2 个是**测试宿主的硬限制**，它造不出 security-scoped 书签）。计数按用例名去重——原始日志每个用例会打两行。**用 Release 配置跑全量单测在设计上不可能**：`ArchiveKitTests` 用了 `@testable import`，它需要 `-enable-testing`（只有 Debug 有），所以「零警告」标准里的 Release 那一半指的是**完整构建**，不是跑测试。
...
面向读者的 UI 另有一条自动通道。`Scripts/verify-ui.sh` 通过一个**仅 DEBUG 编译的 App 内驱动**来操作窗口——因为本该用来打开那两个面板的快捷键（`⇧⌘G` / `⌥⌘G`）无法从外部注入，辅助功能 TCC 拒绝合成按键。驱动会写下结构化状态行，并配一张截图，于是判据是刻意做成双层的：**日志证数据、截图证版式**，只取任何一层都会放过一整类错误。这条通道跑在 **Debug 构建**上，而不是发布产物。批量导出面板（`⇧⌘E`）**已于 2026-09-22 进这条通道**（`ONLY=export`，见测试文档 §18.5）。它的行为原本就有单元测试覆盖（顺序、字节、写失败映射）；新加的这段补的是单测看不见的那部分 —— 面板在真实窗口里的版式，以及一条**「报告 ↔ 磁盘」对照**：报告说写了几个、驱动在磁盘上数出几个、脚本再把同一个目录数第三遍。**两处仍未覆盖**：`running` 中途那一帧（也就是「停止」按钮的落点 —— 30 页只写 200ms，截图竞态赢不了），以及驱动刻意绕过的 `NSOpenPanel` 选目录那一步。

首次阅读提示条也在同一天进了这条通道（`ONLY=hint`）。它的判据刻意做得很薄 —— 只有一个字段 `visible=1` —— 因为「提示条在屏上」与「它是自己冒出来的」在事后**无法区分**：一次性标记与显示是同一段同步代码写的。所以驱动改记 `markerAtAppear`（启动那一刻存储里的值）与 `didAutoShowHintBar`（「本次打开是否自动显示过」、每次打开都会归零的字段），而不去读那个标记。至于提示条**会不会挡住页面**，这一条根本**不做测量** —— 它是布局里的一行、页面拿到的是扣掉它之后的高度，所以那是结构上的性质，不是量出来的性质。

截图证据现在有一部分也被机器读了（`Scripts/page_metrics.swift`）。这一步是被逼出来的：**「铺满宽度」和「铺满宽度但被裁掉两边」在肉眼看来逐像素相同**——连续滚动的行原先取自外层 `GeometryReader` 的宽度（900pt），而滚动视图内容区实宽只有 885pt（系统在「始终显示滚动条」时会给传统滚动条留 15pt 占位），于是每一页左右各丢 7.5pt。探针量的是 **fixture 页脚自带的进度条**（宽度是页宽的已知比例），再拿它跟实测的内容区宽做断言。截图仍然承载只有人才能判断的东西（崩溃询问读起来会不会太硬、空态协不协调）。

fixture 由脚本生成、是确定性的、并且已入库——所以「逐字节页比对」意味着每台机器上都是同样的字节。重新生成它们 = 跑一次脚本 + 跑一次测试。
