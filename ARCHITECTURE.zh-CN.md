...
截至 2026-09-24：**355 用例 / 353 通过 / 2 skip**（两个 skip 是外部样本与合成基准用例，都需要手动开启）。计数按用例名去重——原始日志每个用例会打两行。**用 Release 配置跑全量单测在设计上不可能**：`ArchiveKitTests` 用了 `@testable import`，它需要 `-enable-testing`（只有 Debug 有），所以「零警告」标准里的 Release 那一半指的是**完整构建**，不是跑测试。
